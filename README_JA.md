...
ひとつのコードベースで → Android · Meta Quest · Windows · macOS · Linux · iOS
...
**Love Iwara**（別名 `i_iwara`、**2i**）は、Flutter で構築された [Iwara](https://www.iwara.tv) のサードパーティクライアントです。スマートフォン・タブレット・デスクトップのいずれでも、ネイティブに近いなめらかな体験を提供することを目指しています。しかもすべては単一のコードベースから作られ、**Android・Meta Quest・Windows・macOS・Linux・iOS** に対応しています。
...
<sub>¹ Quest 版は Horizon OS（Android 14 以降）向けの独立した arm64 APK で、Meta Spatial SDK を含みます——詳しくは [Meta Quest / VR](#-meta-quest--vr) を参照してください。通常の Android APK は影響を受けず、引き続き Android 7.0 以降に対応します。</sub>
...
- 読み込み速度のリアルタイム表示
- **平面ディスプレイでのパノラマ表示**——VR180 / 360° のファイルは他では押し潰された 2 つの半分として並んで見えますが、ここでは正しい比率のビューポートになり、ドラッグで見回せます。Iwara のファイルには球面メタデータが一切ないため形式は推測に頼っており、いつでも手動で修正でき、その修正はその動画に対して記憶されます
- **「遠近」**——押し続けると映像を遠ざけたり近づけたりできます（平面動画は映像のズーム、パノラマ動画は視野角）
- **他のアプリで開く**——現在の動画を MX Player / VLC、Skybox や Pigasus といった VR プレイヤー、デスクトップでは任意の外部プレイヤーに渡せます。ローカルまたはダウンロード済みのファイルを優先し、「リンクをコピー」も用意しています
- デスクトップ：ローカルの動画ファイルをウィンドウに**ドラッグ＆ドロップ**するだけですぐ再生

### 🥽 Meta Quest / VR
*Meta Spatial SDK 上に構築された、独立した `quest` APK として配布されます。通常の Android 版はそれらを一切リンクせず、`minSdk` も変更しません。*

**アプリ全体が空間の中に住んでいます。** Quest のホームから起動すると、そのまま常駐の没入空間に入ります。閲覧・検索・コメント・ソフトキーボードまで、アプリのすべてが 2D パネルとして目の前に浮かびます。機能が削られている部分はありません。このパネルは**ゆるやかな曲面**です（30° の弧。既定の幅 1.6 m ならおよそ「3000R」の曲面ディスプレイ相当）。固定されているのは半径ではなく弧の角度なので、ウィンドウをどれだけ広げても狭めても曲がり具合は同じに見えます。

**3 つのウィンドウ、1 組のジェスチャー。** アプリパネル・スクリーン・コントロールパネルはそれぞれ独自の枠を持ちます。グリップトリガーで本体をつかんで移動、四辺で移動、四隅は中心を原点にリサイズ、ウィンドウは常にこちらを向き続けます。サイズと位置は記憶されます。ヘッドトラッキングが安定するまで何も現れません——間違った位置に一瞬出てから飛ぶ、ということが起きません。

**プレイヤーは空間内のスクリーンになります。** 動画を開くと自動的に引き渡されます（オフにも可）。
- **スクリーン形状**——平面、または 3 段階の曲率。距離・幅・画面比はすべて調整でき、**アスペクト比ごとに記憶**されます。4:3 の動画に戻れば、4:3 用に設定した配置がそのまま戻ります。
- **動画フォーマット**——2D / 3D、ハーフ＆フル SBS、ハーフ＆フル OU、180° と 360° エクイレクタングラー。ファイルから自動判別し、手動修正も可能です。EAC と魚眼は認識したうえで非対応と表示し、外部プレイヤーに渡します。
- **再生**——0.5×〜3.0× の速度、ループ、音量、目標時刻と増減を表示するシークプレビュー、スクリーン中央のバッファリング表示、パネル上の時計とバッテリー。
- **空間内の「続きから見る」**——2D アプリと同じセクション分けされた動画プール（ソース一覧・購読・プレイリスト・お気に入り・ダウンロード済み・あとで見る）をサムネイル付きで。空間を離れずに動画を切り替えられ、パネルは読み込み状態を表示し、次が失敗すれば元の動画に戻します。
- **期限切れリンクへの対応**——期限前にソースを更新し、再生中に 404 が返ってきた場合も更新して元の位置から再開します。
- **シーン**——パススルー、または暗転。

**空間ギャラリー。** ギャラリーは 1 枚の大きなステージとフィルムストリップとして開きます。数十枚の画像も、その中に混じった動画も、数十枚のレイヤーではなく 1 つのオブジェクトのままです。スライドショーは 3 / 5 / 10 / 20 秒、画質は標準と原寸、ステージを横にドラッグしてページ送り。縦長画像は 16:9 の枠に収まるので、9:16 のイラストが目の前にそびえ立つことはありません。

**ハンドトラッキングでもコントローラーでも。**
- *ハンドトラッキング*——公式のレイとピンチ。パネル以外の場所でピンチすると、コントロールパネルの表示・非表示が切り替わります。
- *コントローラー*——A/X で再生・一時停止、B/Y でパネルを閉じるかアプリに戻る、Menu で設定。グリップトリガーは狙わなくてもスクリーンをつかめます。スティック左右はシーク（軽く倒すと 5 秒、押し続けると最大で毎秒 15 分まで加速）、上下でスクリーンを遠ざけたり近づけたりします。
- ヘッドセットを外すと一時停止し、着け直すと再開します。システムの再センタリングですべてが目の前に置き直されます。

**初回はまずレッスンから。** ヘッドセットでの初回ガイドは、タッチ画面向けの「ピンチでズーム、長押しで速度変更」をそのまま持ち込んだものではありません——空間には触れる画面そのものが存在しないからです。代わりに**空間動画**と**空間ギャラリー**の 2 コースを用意しました。コントローラーの図がボタンやスティックに合わせてコマ送りで動き、スクリーン・パネル・レイは実際のジオメトリどおりに配置され、コントローラーとハンドトラッキングの両方で実演します。システムの「視差効果を減らす」が有効なら、情報量のある静止画 1 枚だけを描画します。設定からいつでも見直せます。
...
**Quest 対応はすでに実装済みです**——上の [Meta Quest / VR](#-meta-quest--vr) を参照してください。独立した APK として、独立した CI ジョブから配布され、通常の Android 版は一切影響を受けません。

ヘッドセット側で残っている課題：

- 空間パネルの GPU メモリ解放パスが現時点では機能していません。
- 空間シーンは Flutter とメインスレッドを共有しており、そのフレームコストは一度も計測していません。
- アプリパネルは Activity ベースで、公式が「軽い」としている View ベースのパネルへ移行すべきです。
- EAC と魚眼の投影は判別はできますが、自前で描画せず外部プレイヤーに渡しています。

その他：ダウンロードは引き続きベータ版で、Linux はビルドできるもののテスト機がなく未検証のままです。

ご要望があれば [Issue](https://github.com/FoxSensei001/LoveIwara/issues) か [Telegram グループ](https://t.me/+ITH4CV6Z_sc2ZWVl) までどうぞ。
...
# 4. 実行（接続中のデバイスを自動選択）
flutter run --flavor standard   # Android は flavor 必須（下記の注意を参照）
# …またはプラットフォームを指定（デスクトップ / iOS は flavor 不要）：
flutter run -d windows   # macos / linux / ios
```

> [!IMPORTANT]
> **Android には 2 つのプロダクトフレーバーがあります。** `standard` は通常のスマホ / タブレット向け
> （`minSdk 24`、全 ABI）、`quest` は Meta Quest / Horizon OS 向け（`minSdk 34`、arm64 のみ、
> 約 50 MB の Meta Spatial SDK を同梱）です。フレーバーを定義すると AGP には「フレーバーなし」の
> バリアントが存在しなくなるため、**Android の `flutter run` / `flutter build` は必ず `--flavor` を
> 付ける必要があります**。付けないコマンドはそのまま失敗します。
...
### リリースビルド
```bash
flutter build apk --release --flavor standard         # Android APK（スマホ / タブレット）
...
**4. ビルド** —— `flutter build apk --release --flavor standard` を実行すると、`build/app/outputs/flutter-apk/app-standard-release.apk` に出力されます。Quest 版は `flutter build apk --release --flavor quest --target-platform android-arm64` で、`app-arm64-v8a-quest-release.apk` が生成されます。
