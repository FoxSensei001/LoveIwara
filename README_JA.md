...
**アプリ全体が空間の中に住んでいます。** Quest のホームから起動すると、そのまま常駐の没入空間に入ります。閲覧・検索・コメント・ソフトキーボードまで、アプリのすべてが 2D パネルとして目の前に浮かびます。機能が削られている部分はありません。
...
- *コントローラー*——A/X で再生・一時停止、B/Y でパネルを閉じるかアプリに戻る、Menu で設定。グリップトリガーは狙わなくてもスクリーンをつかめます。スティック左右はシーク（軽く倒すと 10 秒、押し続けると最大で毎秒 30 分まで加速）、上下でスクリーンを遠ざけたり近づけたりします。
