...
class MarkdownTranslationController {
  ...
  Future<void> translate(String text, {String? targetLanguage, String? originalText}) async {
    ...
    if (stream != null) {
      _translationStreamSubscription = stream.listen(
        ...
        onError: (error) {
          rawTranslatedText.value = slang.t.common.translateFailedPleaseTryAgainLater;
  ...
  Future<void> _processTranslatedText() async {
    ...
    try {
      // 使用工具类处理翻译文本的格式化
      final processed = await _markdownFormatter.processTranslatedText(rawTranslatedText.value!);
      
      translatedText.value = processed;
      isTranslating.value = false;
      isTranslationComplete.value = true;
    } catch (e) {
      LogUtils.e('格式化翻译文本时发生错误', error: e, tag: 'MarkdownTranslationController');
      translatedText.value = rawTranslatedText.value;
      isTranslating.value = false;
      isTranslationComplete.value = true;
    }
