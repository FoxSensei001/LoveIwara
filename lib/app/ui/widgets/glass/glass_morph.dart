...
/// | 从玻璃里吐出一张选择面板                | 各页自建 Overlay，出入场/手感各演化一套  | `showGlassMenu`       |
///
/// # 最后两行是「关系形变」：跟手 + 融合
///
/// 前面几个原语讲的都是**一块**玻璃自己怎么变，这两个讲的是**手指与玻璃**、
/// **玻璃与玻璃**之间的关系，它们必须成对出现才读得通：
///
/// - **跟手（`GlassSurface.liquidTouch` / `GlassButtonGroup.touchFlex` /
///   `showGlassMenu(touchFlex:)`）**：按住并拖动时整只玻璃顺着手指拉长、垂直
///   方向挤扁，松手弹回。**三处都默认开**——「一块玻璃按下去会不会动」是材质
///   自带的手感，不该由每个调用点各自决定；用户按住 header 上的下拉钮没反应、
///   按住旁边的按钮组却会蠕动，读起来就是「有的是玻璃，有的是塑料」。确实不该
///   动的地方（浮动底栏那种自己管手势的）才显式关掉。
/// - **融合（`GlassBlendGroup`）**：被拉长的那块碰到邻居时，两者之间先长出
///   一段液面颈部再合成一坨，分开时反过来断开。
///
/// 只有跟手没有融合，就是用户 2026-08-23 报的那个问题：**浮动底栏**上长按搜索
/// 圆钮往左拖会和栏目胶囊融在一起（那是包里 `GlassTabBar.bottom` 自带的），
/// 而 **header 上头像圆钮往右拖到分段胶囊上却只是叠上去**——同一套材质语言，
/// 两处却不一致，读起来后者「不是液体，是两块塑料」。
///
/// ## 规矩
///
/// 1. **一行并排的 chrome 必须收进同一个 `GlassBlendGroup`**。`GlassHeaderOverlay`
///    已经替所有 `liquid: true` 的 header 行包好了，页面不用自己写；自己摆的
///    浮动工具条（底部动作坞、悬浮小工具栏）要自己包。
/// 2. **间距与 blend 是一对**。默认 8px 间距配 [GlassTokens.chromeBlend]（8）：
...
///
/// # ⛔ 「从玻璃里吐出一张面板」只有 `showGlassMenu` 一个出口
///
/// 下拉选择、排序切换、更多菜单、长按动作表——凡是「点一下某块玻璃，底下弹出
/// 一张可选列表」的，一律走 `showGlassMenu`，不要再自己搭 `OverlayEntry` +
/// `CompositedTransformFollower` + 落点算术。自建的那份必然会漏掉这一整套：
/// 材质从触发件取样、Telegram 式卷开出入场（**不含任何 `Opacity`**，否则折射
/// 会断）、跟手形变、滑动取焦、下方放不下时翻到上方、无障碍语义。
///
/// 2026-08-23 订阅页的「特别关注」选人面板就是这么收编的——它自建的那份用
/// `FadeTransition` 做出入场，正好踩在那条折射告警上，而同一行 header 上「更多」
/// 吐出来的菜单一切正常。行首要放头像/自定义图标时用
/// `GlassMenuOption.leading`（固定槽位，能被静态量宽所以不影响跟手形变），
/// 行级长按用 `GlassMenuOption.onLongPress`。
///
