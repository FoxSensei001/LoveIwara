...
class Oreno3dHtmlParser {
  ...
  static Map<String, int> _extractPaginationInfo(Document document) {
    ...
    // 查找所有页码链接
    final pageLinks = paginationElement.querySelectorAll(
    ...
    // 也检查"››"链接中的最后页码
    final lastPageLink = paginationElement.querySelector(
  ...
  static String _extractSortType(Document document) {
    final activeSortButton = document.querySelector(
      '.main-sort-button.sort-choice',
    );
  ...
  /// 解析视频详情页面HTML，提取详细信息
  static Oreno3dVideoDetail? parseVideoDetail(
    String htmlContent,
    String videoId,
  ) {
    final document = html_parser.parse(htmlContent);
    ...
    try {
      ...
      // 提取缩略图URL
      final imgElement = articleElement.querySelector(
        '.video-figure img.video-img',
      );
      final thumbnailUrl = imgElement?.attributes['src'] ?? '';
      ...
      // 提取播放链接
      final playLinkElement = articleElement.querySelector(
        '.video-figure a.pop_separate, a.video-watch-btn2[target="_blank"]',
      );
      final playUrl = playLinkElement?.attributes['href'] ?? '';

      // 提取发布日期和时间
      DateTime? publishedAt;
      final dateElements = articleElement.querySelectorAll(
        '.video-views .f-label-in',
      );
      ...
      int viewCount = 0;
      int favoriteCount = 0;
      final viewElements = articleElement.querySelectorAll(
        '.video-views .f-label-in',
      );
      ...
      if (authorSection != null) {
        ...
        if (authorH2?.text.trim() == '作成者：') {
          ...
          if (authorLink != null) {
            ...
            // 确保获取正确的文本内容，排除图标
            final authorNameElement = authorLink.querySelector(
              'div.video-center',
            );
            ...
            if (authorName.isNotEmpty) {
              author = Oreno3dLinkItem(
                id: authorId,
                name: authorName,
                url: authorUrl.startsWith('http')
                    ? authorUrl
                    : 'https://oreno3d.com$authorUrl',
              );
            }
          }
        }
      ...
      for (final section in sections) {
        ...
        if (h2Element?.text.trim() == '原作：') {
          ...
          if (originLink != null) {
            ...
            // 确保获取正确的文本内容，排除图标
            final originNameElement = originLink.querySelector(
              'div.video-center',
            );
            ...
            if (originName.isNotEmpty) {
              origin = Oreno3dLinkItem(
                id: originId,
                name: originName,
                url: originUrl.startsWith('http')
                    ? originUrl
                    : 'https://oreno3d.com$originUrl',
              );
            }
          }
          break;
      ...
      for (final section in sections) {
        ...
        if (h2Element?.text.trim() == 'キャラ：') {
          ...
          for (final characterLink in characterLinks) {
            ...
            // 确保获取正确的文本内容，排除图标
            final characterNameElement = characterLink.querySelector(
              'div.video-center',
            );
            ...
            if (characterName.isNotEmpty) {
              characters.add(
                Oreno3dLinkItem(
                  id: characterId,
                  name: characterName,
                  url: characterUrl.startsWith('http')
                      ? characterUrl
                      : 'https://oreno3d.com$characterUrl',
                ),
              );
            }
          }
      ...
      for (final section in sections) {
        ...
        if (h2Element?.text.trim() == 'タグ：') {
          ...
          for (final tagLink in tagLinks) {
            ...
            final tagName =
                tagLink.querySelector('.tag-text')?.text.trim() ?? '';
            ...
            if (tagName.isNotEmpty) {
              tags.add(
                Oreno3dLinkItem(
                  id: tagId,
                  name: tagName,
                  url: tagUrl.startsWith('http')
                      ? tagUrl
                      : 'https://oreno3d.com$tagUrl',
                ),
              );
            }
          }
      ...
      // 提取作者评论
      final commentElement = articleElement.querySelector(
