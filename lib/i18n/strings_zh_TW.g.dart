...
extension on TranslationsZhTw {
	dynamic _flatMapFunction(String path) {
		switch (path) {
			...
			case 'tutorial.exampleAuthorInfoRow': return '範例：作者資訊行';
			...
			case 'common.mobileSaveImageIsUnderDevelopment': return '行動端的儲存圖片功能尚在開發中';
			...
			case 'common.createPlayList': return '建立播放清單';
			case 'common.create': return '建立';
			...
			case 'common.showOriginalText': return '顯示原始文字';
			case 'common.showProcessedText': return '顯示處理後文字';
			...
			case 'common.createPost': return '建立投稿';
			...
			case 'common.disableFloatingButtons': return '停用浮動按鈕';
			case 'common.enabledFloatingButtons': return '已啟用浮動按鈕';
			case 'common.disabledFloatingButtons': return '已停用浮動按鈕';
			...
			case 'common.selectDateRangeHint': return '選擇日期範圍，預設選擇最近30天';
			...
			case 'common.createTimeDesc': return '建立時間倒序';
			case 'common.createTimeAsc': return '建立時間正序';
			...
			case 'common.parseExceptionDestopHint': return ' - 桌面端使用者可以在設定中配置代理';
			...
			case 'auth.usernameOrEmail': return '使用者名稱或電子郵件';
			case 'auth.pleaseEnterUsernameOrEmail': return '請輸入使用者名稱或電子郵件';
			...
			case 'errors.sessionExpired': return '工作階段已過期';
			...
			case 'errors.failedToSaveCredentials': return '無法安全儲存登入資訊';
			case 'errors.failedToLoadSavedCredentials': return '載入儲存的登入資訊失敗';
			...
			case 'errors.network.connectionFailed': return '連線失敗';
			case 'errors.network.sslConnectionFailed': return 'SSL連線失敗，請檢查網路設定';
			case 'friends.clickToRestoreFriend': return '點擊復原朋友';
			...
			case 'favorites.clickToRestoreFavorite': return '點擊復原最愛';
			...
			case 'playList.yesCreateListWillAlwaysExistAndVisibleToEveryone': return '沒錯...建立的播放清單會一直存在且對所有人可見';
			...
			case 'playList.createNewPlaylist': return '建立新播放清單';
			...
			case 'search.googleSearch': return 'Google搜尋';
			case 'search.googleSearchHint': return ({required Object webName}) => '${webName} 的搜尋功能不好用？嘗試Google搜尋！';
			case 'search.googleSearchDescription': return '借助Google搜尋的 :site 搜尋運算符，您可以透過外部引擎來對站內的內容進行檢索，此功能在搜尋 影片、圖庫、播放清單、使用者 時非常有用。';
			case 'search.googleSearchKeywordsHint': return '輸入要搜尋的關鍵字';
			case 'search.openLinkJump': return '打開連結跳轉';
			case 'search.googleSearchButton': return 'Google搜尋';
			case 'search.pleaseEnterSearchKeywords': return '請輸入搜尋關鍵字';
			...
			case 'settings.showVideoProgressBottomBarWhenToolbarHiddenDesc': return '此設定決定是否在工具列隱藏時顯示底部進度條';
			...
			case 'settings.invalidProxyAddressFormatPleaseUseTheFormatOfIpPortOrDomainNamePort': return '無效的代理伺服器地址格式，請使用 IP:埠 或 域名:埠 格式。';
			...
			case 'settings.currentVersion': return '目前版本';
			...
			case 'settings.minVersionUpdateRequired': return '目前版本過低，請盡快更新';
			...
			case 'settings.autoRecordHistoryDesc': return '自動記錄您觀看過的影片和圖庫等資訊';
			case 'settings.showUnprocessedMarkdownText': return '顯示未處理文字';
			case 'settings.showUnprocessedMarkdownTextDesc': return '顯示Markdown的原始文字';
			case 'settings.markdown': return 'Markdown';
			case 'settings.activeBackgroundPrivacyMode': return '隱私模式';
			case 'settings.activeBackgroundPrivacyModeDesc': return '禁止截圖、後台執行時隱藏畫面...';
			case 'settings.privacy': return '隱私';
			case 'settings.forum': return '論壇';
			case 'settings.disableForumReplyQuote': return '停用論壇回覆引用';
			case 'settings.disableForumReplyQuoteDesc': return '停用論壇回覆時攜帶被回覆樓層資訊';
			case 'settings.theaterMode': return '劇院模式';
			case 'settings.theaterModeDesc': return '開啟後，播放器背景會被設定為影片封面的模糊版本';
			case 'settings.appLinks': return '應用連結';
			case 'settings.defaultBrowser': return '預設瀏覽';
			case 'settings.defaultBrowserDesc': return '請在系統設定中打開預設連結配置項，並添加iwara.tv網站連結';
			...
			case 'settings.customColors': return '自訂顏色';
			case 'settings.pickColor': return '選擇顏色';
			case 'settings.cancel': return '取消';
			case 'settings.confirm': return '確認';
			case 'settings.noCustomColors': return '沒有自訂顏色';
			case 'settings.recordAndRestorePlaybackProgress': return '記錄和復原播放進度';
			...
			case 'settings.enableVibrationDesc': return '啟用應用互動時的震動回饋';
			...
			case 'settings.theaterModelHasPerformanceIssuesAndIDontKnowHowToFixItNowIfYouRRuningOnDeskTopYouCanOpenIt': return '行動端開啟劇院模式可能會造成性能問題，可酌情開啟。';
			...
			case 'settings.fullscreenOrientation': return '豎屏進入全螢幕後的螢幕方向';
			case 'settings.fullscreenOrientationDesc': return '此設定決定豎屏進入全螢幕時螢幕的預設方向（僅行動端有效）';
			case 'settings.fullscreenOrientationLeftLandscape': return '左側橫螢幕';
			case 'settings.fullscreenOrientationRightLandscape': return '右側橫螢幕';
			...
			case 'settings.enableMouseHoverShowToolbar': return '滑鼠懸停時顯示工具列';
			case 'settings.enableMouseHoverShowToolbarInfo': return '開啟後，當滑鼠懸停在播放器上移動時會自動顯示工具列，停止移動3秒後自動隱藏';
			case 'settings.enableHorizontalDragSeek': return '橫向滑動調整進度';
			case 'settings.audioVideoConfig': return '影音配置';
			case 'settings.expandBuffer': return '擴大緩衝區';
			case 'settings.expandBufferInfo': return '開啟後緩衝區增大，載入時間變長但播放更流暢';
			case 'settings.videoSyncMode': return '影片同步模式';
			case 'settings.videoSyncModeSubtitle': return '影音同步策略';
			...
			case 'settings.useOpenSLESAudioOutput': return '使用OpenSLES音訊輸出';
			case 'settings.useOpenSLESAudioOutputInfo': return '使用低延遲音訊輸出，可能提高音頻效能';
			case 'settings.videoSyncAudio': return '音訊同步';
			...
			case 'settings.videoSyncDisplayVdrop': return '顯示丟影片幀';
			case 'settings.videoSyncDisplayAdrop': return '顯示丟音訊幀';
			...
			case 'settings.chatSettings.configureYourChatSettings': return '配置您的聊天設定';
			...
			case 'settings.hardwareDecodingNo': return '停用';
			case 'settings.hardwareDecodingYes': return '強制啟用';
			case 'settings.cdnDistributionStrategy': return '內容分流策略';
			case 'settings.cdnDistributionStrategyDesc': return '選擇影片來源伺服器的分流方式，可優化載入速度';
			case 'settings.cdnDistributionStrategyLabel': return '分流策略';
			case 'settings.cdnDistributionStrategyNoChange': return '不更動（使用原本的伺服器）';
			case 'settings.cdnDistributionStrategyAuto': return '自動選擇（最快的伺服器）';
			case 'settings.cdnDistributionStrategySpecial': return '指定伺服器';
			case 'settings.cdnSpecialServer': return '指定伺服器';
			case 'settings.cdnRefreshServerListHint': return '請先點選下方按鈕重新整理伺服器清單';
			case 'settings.cdnRefreshButton': return '重新整理';
			case 'settings.cdnFastRingServers': return '快速環伺服器';
			case 'settings.cdnRefreshServerListTooltip': return '重新整理伺服器清單';
			case 'settings.cdnSpeedTestButton': return '測速';
			case 'settings.cdnSpeedTestingButton': return ({required Object count}) => '測速中 (${count})';
			case 'settings.cdnNoServerDataHint': return '查無伺服器資料，請點選重新整理按鈕';
			case 'settings.cdnTestingStatus': return '測速中';
			case 'settings.cdnUnreachableStatus': return '無法連線';
			case 'settings.cdnNotTestedStatus': return '尚未測速';
			case 'settings.downloadSettings.downloadSettings': return '下載設定';
			case 'settings.downloadSettings.storagePermissionStatus': return '儲存權限狀態';
			case 'settings.downloadSettings.accessPublicDirectoryNeedStoragePermission': return '訪問公共目錄需要儲存權限';
			case 'settings.downloadSettings.checkingPermissionStatus': return '檢查權限狀態...';
			case 'settings.downloadSettings.storagePermissionGranted': return '已授權儲存權限';
			case 'settings.downloadSettings.storagePermissionNotGranted': return '需要儲存權限';
			case 'settings.downloadSettings.storagePermissionGrantSuccess': return '儲存權限授權成功';
			case 'settings.downloadSettings.storagePermissionGrantFailedButSomeFeaturesMayBeLimited': return '儲存權限授權失敗，部分功能可能受限';
			case 'settings.downloadSettings.grantStoragePermission': return '授權儲存權限';
			case 'settings.downloadSettings.customDownloadPath': return '自訂下載位置';
			case 'settings.downloadSettings.customDownloadPathDescription': return '啟用後可以為下載的檔案選擇自訂儲存位置';
			case 'settings.downloadSettings.customDownloadPathTip': return '💡 提示：選擇公共目錄（如下載資料夾）需要授予儲存權限，建議優先使用推薦路徑';
			case 'settings.downloadSettings.androidWarning': return 'Android提示：避免選擇公共目錄（如下載資料夾），建議使用應用程式專用目錄以確儲存取權限。';
			...
			case 'settings.downloadSettings.enableCustomDownloadPath': return '啟用自訂下載路徑';
			case 'settings.downloadSettings.disableCustomDownloadPath': return '關閉時使用應用程式預設路徑';
			case 'settings.downloadSettings.customDownloadPathLabel': return '自訂下載路徑';
			...
			case 'settings.downloadSettings.filenameTemplateDescription': return '自訂下載檔案的命名規則，支援變數取代';
			...
			case 'settings.downloadSettings.testPassed': return '項透過';
			case 'settings.downloadSettings.testFailed': return '測試失敗';
			case 'settings.downloadSettings.testStoragePermissionCheck': return '儲存權限檢查';
			case 'settings.downloadSettings.testStoragePermissionGranted': return '已獲得儲存權限';
			case 'settings.downloadSettings.testStoragePermissionMissing': return '缺少儲存權限，部分功能可能受限';
			...
			case 'settings.downloadSettings.testVideoTemplate': return '影片模板';
			...
			case 'settings.downloadSettings.testDirectoryCreation': return '目錄建立';
			...
			case 'settings.downloadSettings.lackStoragePermission': return '缺少儲存權限';
			...
			case 'settings.downloadSettings.requiresStoragePermission': return '需要儲存權限才能存取';
			...
			case 'oreno3d.errors.connectionError': return '網路連接錯誤，請檢查網路設定';
			case 'oreno3d.errors.networkRequestFailed': return '網路請求失敗';
			case 'oreno3d.errors.searchVideoError': return '搜尋影片時發生未知錯誤';
			case 'oreno3d.errors.getPopularVideoError': return '獲取熱門影片時發生未知錯誤';
			case 'oreno3d.errors.getVideoDetailError': return '獲取影片詳情時發生未知錯誤';
			case 'oreno3d.errors.parseVideoDetailError': return '獲取並解析影片詳情時發生未知錯誤';
			case 'oreno3d.errors.downloadFileError': return '下載文件時發生未知錯誤';
			case 'oreno3d.loading.gettingVideoInfo': return '正在獲取影片資訊...';
			case 'oreno3d.loading.cancel': return '取消';
			case 'oreno3d.messages.videoNotFoundOrDeleted': return '影片不存在或已被刪除';
			case 'oreno3d.messages.unableToGetVideoPlayLink': return '無法獲取影片播放連結';
			case 'oreno3d.messages.getVideoDetailFailed': return '獲取影片詳情失敗';
			...
			case 'subscriptions.selectUser': return '選擇使用者';
			case 'subscriptions.noSubscribedUsers': return '尚無已訂閱使用者';
			case 'subscriptions.showAllSubscribedUsersContent': return '顯示所有已訂閱使用者的內容';
			...
			case 'videoDetail.home': return '首頁';
			...
			case 'videoDetail.resourceDeleted': return '這個影片好像被刪除了 :/';
			...
			case 'videoDetail.videoLoadError': return '影片載入錯誤';
			...
			case 'videoDetail.player.errorWhileLoadingVideoSource': return '在載入影片來源時出現了錯誤';
			case 'videoDetail.player.errorWhileSettingUpListeners': return '在設定監聽器時出現了錯誤';
			case 'videoDetail.skeleton.fetchingVideoInfo': return '取得影片資訊中...';
			case 'videoDetail.skeleton.fetchingVideoSources': return '取得影片來源中...';
			case 'videoDetail.skeleton.loadingVideo': return '正在載入影片...';
			case 'videoDetail.skeleton.applyingSolution': return '正在應用此解析度...';
			case 'videoDetail.skeleton.addingListeners': return '正在添加監聽器...';
			case 'videoDetail.skeleton.successFecthVideoDurationInfo': return '成功獲取影片資源，開始載入影片...';
			case 'videoDetail.skeleton.successFecthVideoHeightInfo': return '載入完成';
			case 'videoDetail.cast.dlnaCast': return '投影';
			case 'videoDetail.cast.unableToStartCastingSearch': return ({required Object error}) => '啟動投影搜尋失敗: ${error}';
			case 'videoDetail.cast.startCastingTo': return ({required Object deviceName}) => '開始投影到 ${deviceName}';
			case 'videoDetail.cast.castFailed': return ({required Object error}) => '投影失敗: ${error}\n請嘗試重新搜尋裝置或切換網路';
			case 'videoDetail.cast.castStopped': return '已停止投影';
			case 'videoDetail.cast.deviceTypes.mediaRenderer': return '媒體播放器';
			case 'videoDetail.cast.deviceTypes.mediaServer': return '媒體伺服器';
			case 'videoDetail.cast.deviceTypes.internetGatewayDevice': return '路由器';
			case 'videoDetail.cast.deviceTypes.basicDevice': return '基礎裝置';
			case 'videoDetail.cast.deviceTypes.dimmableLight': return '智慧燈';
			case 'videoDetail.cast.deviceTypes.wlanAccessPoint': return '無線接入點';
			case 'videoDetail.cast.deviceTypes.wlanConnectionDevice': return '無線連接裝置';
			case 'videoDetail.cast.deviceTypes.printer': return '印表機';
			case 'videoDetail.cast.deviceTypes.scanner': return '掃描器';
			case 'videoDetail.cast.deviceTypes.digitalSecurityCamera': return '攝影機';
			case 'videoDetail.cast.deviceTypes.unknownDevice': return '未知裝置';
			case 'videoDetail.cast.currentPlatformNotSupported': return '目前平台不支援投影功能';
			case 'videoDetail.cast.unableToGetVideoUrl': return '無法獲取影片網址，請稍後重試';
			case 'videoDetail.cast.stopCasting': return '停止投影';
			case 'videoDetail.cast.dlnaCastSheet.title': return '遠端投影';
			case 'videoDetail.cast.dlnaCastSheet.close': return '關閉';
			case 'videoDetail.cast.dlnaCastSheet.searchingDevices': return '正在搜尋裝置...';
			case 'videoDetail.cast.dlnaCastSheet.searchPrompt': return '點擊搜尋按鈕重新搜尋投影裝置';
			case 'videoDetail.cast.dlnaCastSheet.searching': return '搜尋中';
			case 'videoDetail.cast.dlnaCastSheet.searchAgain': return '重新搜尋';
			case 'videoDetail.cast.dlnaCastSheet.noDevicesFound': return '未發現投影裝置\n請確保裝置在同一網路下';
			case 'videoDetail.cast.dlnaCastSheet.searchingDevicesPrompt': return '正在搜尋裝置，請稍候...';
			case 'videoDetail.cast.dlnaCastSheet.cast': return '投影';
			case 'videoDetail.cast.dlnaCastSheet.connectedTo': return ({required Object deviceName}) => '已連接到: ${deviceName}';
			case 'videoDetail.cast.dlnaCastSheet.notConnected': return '未連接裝置';
			case 'videoDetail.cast.dlnaCastSheet.stopCasting': return '停止投影';
			case 'videoDetail.likeAvatars.dialogTitle': return '誰悄悄地喜歡';
			case 'videoDetail.likeAvatars.dialogDescription': return '好奇他們是誰？來翻翻這本「按讚相簿」吧～';
			case 'videoDetail.likeAvatars.closeTooltip': return '關閉';
			case 'videoDetail.likeAvatars.retry': return '重試';
			case 'videoDetail.likeAvatars.noLikesYet': return '還沒有人出現在這裡，來當第一個吧！';
			case 'videoDetail.likeAvatars.pageInfo': return ({required Object page, required Object totalPages, required Object totalCount}) => '第 ${page} / ${totalPages} 頁 · 共 ${totalCount} 人';
			case 'videoDetail.likeAvatars.prevPage': return '上一頁';
			case 'videoDetail.likeAvatars.nextPage': return '下一頁';
			case 'share.sharePlayList': return '分享播放列表';
			case 'share.wowDidYouSeeThis': return '哇哦，你看過這個嗎？';
			...
			case 'share.shareAsText': return '分享為文字';
			case 'share.shareAsImageDesc': return '將影片封面分享為圖片';
			case 'share.shareAsTextDesc': return '將影片詳情分享為文字';
			case 'share.shareAsImageFailed': return '分享影片封面為圖片失敗，請稍後再試';
			case 'share.shareAsTextFailed': return '分享影片詳情為文字失敗，請稍後再試';
			...
			case 'share.shareUser': return '分享使用者';
			case 'share.userNameIs': return '使用者名稱字叫做';
			case 'share.userAuthorIs': return '使用者作者是';
			...
			case 'markdown.internalLink': return '站內連結';
			case 'markdown.supportAutoConvertLinkBelow': return '支援自動轉換以下類型的連結：';
			case 'markdown.convertLinkExample': return '🎬 影片連結\n🖼️ 圖片連結\n👤 使用者連結\n📌 論壇連結\n🎵 播放列錶連結\n💬 帖子連結';
			case 'markdown.mentionUser': return '提及使用者';
			case 'markdown.mentionUserDescription': return '輸入@後跟使用者名稱，將自動轉換為使用者連結';
			...
			case 'markdown.textStyle': return '文字樣式';
			case 'markdown.textStyleDescription': return '使用特殊符號包圍文本來改變樣式';
			case 'markdown.textStyleSyntax': return '**粗體文字**\n*斜體文字*\n~~刪除線文字~~\n`程式碼文字`';
			case 'markdown.quote': return '引用';
			case 'markdown.quoteDescription': return '使用 > 符號建立引用，多個 > 建立多級引用';
			case 'markdown.quoteSyntax': return '> 這是一級引用\n>> 這是二級引用';
			case 'markdown.list': return '列表';
			case 'markdown.listDescription': return '使用數字+點號建立有序列表，使用 - 建立無序列表';
			case 'markdown.listSyntax': return '1. 第一項\n2. 第二項\n\n- 無序項\n  - 子項\n  - 另一個子項';
			case 'markdown.linkAndImage': return '連結與圖片';
			case 'markdown.linkAndImageDescription': return '連結格式：[文字](URL)\n圖片格式：![描述](URL)';
			case 'markdown.linkAndImageSyntax': return ({required Object link, required Object imgUrl}) => '[連結文字](${link})\n![圖片描述](${imgUrl})';
			case 'markdown.title': return '標題';
			case 'markdown.titleDescription': return '使用 # 號建立標題，數量表示級別';
			case 'markdown.titleSyntax': return '# 一級標題\n## 二級標題\n### 三級標題';
			case 'markdown.separator': return '分隔線';
			case 'markdown.separatorDescription': return '使用三個或更多 - 號建立分隔線';
			...
			case 'forum.createPost': return '建立帖子';
			...
			case 'forum.createThread': return '建立主題';
			...
			case 'forum.leafNames.feedback': return '回饋';
			...
			case 'forum.leafDescriptions.feedback': return '對網站功能和服務的回饋';
			case 'forum.leafDescriptions.support': return '幫助解決網站相關問題';
			case 'forum.leafDescriptions.general': return '討論任何話題';
			case 'forum.leafDescriptions.guides': return '分享你的經驗和教學';
			...
			case 'notifications.errors.unsupportedNotificationType': return '暫不支援的通知類型';
			case 'notifications.errors.unknownUser': return '未知使用者';
			case 'notifications.errors.unsupportedNotificationTypeWithType': return ({required Object type}) => '暫不支援的通知類型: ${type}';
			case 'notifications.errors.unknownNotificationType': return '未知通知類型';
			case 'notifications.notifications': return '通知';
			case 'notifications.profile': return '個人首頁';
			case 'notifications.postedNewComment': return '發表了評論';
			case 'notifications.notifiedOn': return '在您的個人首頁上發表了評論';
			...
			case 'notifications.copyInfoToClipboard': return '複製通知資訊到剪貼簿';
			...
			case 'notifications.dueToLackOfNotificationTypeDetails': return '通知類型的詳細資訊不足，目前支援的類型可能沒有覆蓋到您目前收到的消息';
			case 'notifications.helpUsImproveNotificationTypeSupport': return '如果您願意幫助我們完善通知類型的支援：';
			case 'notifications.helpUsImproveNotificationTypeSupportLongText': return '1. 📋 複製通知資訊\n2. 🐞 前往項目倉庫提交 issue\n\n⚠️ 注意：通知資訊可能包含個人隱私，如果你不想公開，也可以透過郵件發送給項目作者。';
			case 'notifications.goToRepository': return '前往項目倉庫';
			case 'notifications.copy': return '複製';
			case 'notifications.commentApproved': return '評論已透過';
			case 'notifications.repliedYourProfileComment': return '回覆了您的個人首頁評論';
			...
			case 'notifications.kProfile': return '首頁';
			case 'notifications.kThread': return '主題';
			case 'notifications.kPost': return '投稿';
			case 'notifications.kCommentSection': return '下的評論';
			case 'notifications.kApprovedComment': return '評論已透過';
			case 'notifications.kApprovedVideo': return '影片已透過';
			case 'notifications.kApprovedGallery': return '圖庫已透過';
			case 'notifications.kApprovedThread': return '主題已審核';
			case 'notifications.kApprovedPost': return '投稿已審核';
			case 'notifications.kApprovedForumPost': return '論壇發言審核透過';
			case 'notifications.kRejectedContent': return '內容審核被拒絕';
			case 'notifications.kUnknownType': return '未知通知類型';
			case 'conversation.errors.pleaseSelectAUser': return '請選擇一個使用者';
			case 'conversation.errors.pleaseEnterATitle': return '請輸入標題';
			case 'conversation.errors.clickToSelectAUser': return '點擊選擇使用者';
			case 'conversation.errors.loadFailedClickToRetry': return '載入失敗,點擊重試';
			case 'conversation.errors.loadFailed': return '載入失敗';
			case 'conversation.errors.clickToRetry': return '點擊重試';
			case 'conversation.errors.noMoreConversations': return '沒有更多消息了';
			case 'conversation.conversation': return '工作階段';
			case 'conversation.startConversation': return '發起工作階段';
			case 'conversation.noConversation': return '暫無工作階段';
			case 'conversation.selectFromLeftListAndStartConversation': return '從左側列表選擇一個工作階段開始聊天';
			case 'conversation.title': return '標題';
			case 'conversation.body': return '內容';
			case 'conversation.selectAUser': return '選擇使用者';
			case 'conversation.searchUsers': return '搜尋使用者...';
			case 'conversation.tmpNoConversions': return '暫無工作階段';
			case 'conversation.deleteThisMessage': return '刪除此消息';
			case 'conversation.deleteThisMessageSubtitle': return '此操作不可復原';
			...
			case 'splash.loading': return '載入中...';
			case 'splash.ready': return '準備完成';
			case 'splash.initializingMessageService': return '初始化消息服務中...';
			case 'download.errors.imageModelNotFound': return '圖庫資訊不存在';
			case 'download.errors.downloadFailed': return '下載失敗';
			case 'download.errors.videoInfoNotFound': return '影片資訊不存在';
			...
			case 'download.errors.userPausedDownload': return '使用者暫停下載';
			...
			case 'download.errors.sslHandshakeFailed': return 'SSL握手失敗，請檢查網路環境';
			case 'download.errors.connectionFailed': return '連接失敗，請檢查網路';
			...
			case 'download.errors.openFolderFailed': return '打開資料夾失敗';
			case 'download.errors.openFolderFailedWithMessage': return ({required Object message}) => '打開資料夾失敗: ${message}';
			...
			case 'download.errors.noDownloadSourceNowPleaseWaitInfoLoaded': return '暫無下載源，請等待資訊載入完成後重試';
			...
			case 'download.errors.unsupportedImageFormat': return ({required Object format}) => '不支援的圖片格式: ${format}';
			case 'download.errors.deleteFileError': return '文件刪除失敗，可能是因為文件被占用';
			case 'download.errors.deleteTaskError': return '任務刪除失敗';
			case 'download.errors.taskNotFound': return '任務未找到';
			case 'download.errors.canNotRefreshVideoTask': return '無法重新整理影片任務';
			case 'download.errors.taskAlreadyProcessing': return '任務已處理中';
			case 'download.errors.failedToLoadTasks': return '載入任務失敗';
			...
			case 'download.errors.unsupportedImageFormatWithMessage': return ({required Object extension}) => '不支援的圖片格式: ${extension}, 可以嘗試下載到裝置上查看';
			case 'download.errors.imageLoadFailed': return '圖片載入失敗';
			case 'download.downloadList': return '下載列表';
			case 'download.viewDownloadList': return '查看下載列表';
			...
			case 'download.showInFolder': return '在資料夾中顯示';
			...
			case 'download.saveToAppDirectory': return '儲存到應用程式目錄';
			case 'download.alreadyDownloadedWithQuality': return '已有相同清晰度的任務，是否繼續下載？';
			case 'download.alreadyDownloadedWithQualities': return ({required Object qualities}) => '此視頻已有${qualities}清晰度的任務，是否繼續下載？';
			case 'download.otherQualities': return '其他清晰度';
			case 'favorite.errors.addFailed': return '追加失敗';
			case 'favorite.errors.addSuccess': return '追加成功';
			case 'favorite.errors.deleteFolderFailed': return '刪除資料夾失敗';
			case 'favorite.errors.deleteFolderSuccess': return '刪除資料夾成功';
			...
			case 'favorite.createFolderSuccess': return '資料夾建立成功';
			case 'favorite.createFolderFailed': return '建立資料夾失敗';
			case 'favorite.createFolder': return '建立資料夾';
			case 'favorite.enterFolderName': return '輸入資料夾名稱';
			case 'favorite.enterFolderNameHere': return '在此輸入資料夾名稱...';
			case 'favorite.create': return '建立';
			case 'favorite.items': return '項目';
			case 'favorite.newFolderName': return '新資料夾';
			case 'favorite.searchFolders': return '搜尋資料夾...';
			case 'favorite.searchItems': return '搜尋項目...';
			case 'favorite.createdAt': return '建立時間';
			case 'favorite.myFavorites': return '我的收藏';
			case 'favorite.deleteFolderTitle': return '刪除資料夾';
			case 'favorite.deleteFolderConfirmWithTitle': return ({required Object title}) => '確定要刪除 ${title} 資料夾嗎？';
			...
			case 'favorite.localizeFavorite': return '本機收藏';
			...
			case 'favorite.searchTags': return '搜尋標籤';
			case 'translation.currentService': return '目前服務';
			case 'translation.testConnection': return '測試連接';
			case 'translation.testConnectionSuccess': return '測試連線成功';
			case 'translation.testConnectionFailed': return '測試連線失敗';
			case 'translation.testConnectionFailedWithMessage': return ({required Object message}) => '測試連線失敗: ${message}';
			case 'translation.translation': return '翻譯';
			case 'translation.needVerification': return '需要驗證';
			case 'translation.needVerificationContent': return '請先透過連接測試才能啟用AI翻譯';
			case 'translation.confirm': return '確定';
			case 'translation.disclaimer': return '使用須知';
			case 'translation.riskWarning': return '風險提示';
			case 'translation.dureToRisk1': return '由於評論等文字為使用者生成，可能包含違反AI服務商內容政策的內容';
			case 'translation.dureToRisk2': return '不當內容可能導致API金鑰封禁或服務終止';
			...
			case 'translation.maxTokensHintText': return '例如：32000';
			...
			case 'translation.disabled': return '已停用';
			...
			case 'translation.information': return '資訊';
			case 'translation.viewRawResponse': return '查看原始響應';
			case 'translation.pleaseCheckInputParametersFormat': return '請檢查輸入參數格式';
			case 'translation.pleaseFillInAPIAddressModelNameAndKey': return '請填寫API地址、模型名稱和金鑰';
			...
			case 'translation.configuredKey': return '已配置金鑰';
			case 'translation.notConfiguredKey': return '未配置金鑰';
			case 'translation.authenticationStatus': return '認證狀態';
			case 'translation.thisFieldCannotBeEmpty': return '此欄位不能為空';
			case 'translation.apiKey': return 'API金鑰';
			case 'translation.apiKeyCannotBeEmpty': return 'API金鑰不能為空';
			...
			case 'translation.connectionFailedForMessage': return ({required Object message}) => '連線失敗: ${message}';
			case 'translation.aiTranslationNotEnabledHint': return 'AI翻譯未啟用，請在設定中啟用';
			case 'translation.goToSettings': return '前往設定';
			case 'translation.disableAITranslation': return '停用AI翻譯';
			...
			case 'translation.onlyOpenAIAPISupported': return '目前僅支援OpenAI相容的API格式（application/json請求體格式）';
			case 'translation.streamingTranslation': return '流式翻譯';
			case 'translation.streamingTranslationSupported': return '支援流式翻譯';
			case 'translation.streamingTranslationNotSupported': return '不支援流式翻譯';
			case 'translation.streamingTranslationDescription': return '流式翻譯可以在翻譯過程中即時顯示結果，提供更好的使用者體驗';
			...
			case 'translation.streamingTranslationWarning': return '注意：此功能需要API服務支援流式傳輸，部分模型可能不支援';
			case 'translation.translationService': return '翻譯服務';
			case 'translation.translationServiceDescription': return '選擇您偏好的翻譯服務';
			case 'translation.googleTranslation': return 'Google 翻譯';
			case 'translation.googleTranslationDescription': return '免費的線上翻譯服務，支援多種語言';
			case 'translation.aiTranslation': return 'AI 翻譯';
			case 'translation.aiTranslationDescription': return '基於大語言模型的智慧翻譯服務';
			case 'translation.deeplxTranslation': return 'DeepLX 翻譯';
			case 'translation.deeplxTranslationDescription': return 'DeepL 翻譯的開源實現，提供高品質翻譯';
			case 'translation.googleTranslationFeatures': return '特性';
			case 'translation.freeToUse': return '免費使用';
			case 'translation.freeToUseDescription': return '無需配置，開箱即用';
			case 'translation.fastResponse': return '快速響應';
			case 'translation.fastResponseDescription': return '翻譯速度快，延遲低';
			case 'translation.stableAndReliable': return '穩定可靠';
			case 'translation.stableAndReliableDescription': return '基於Google官方API';
			case 'translation.enabledDefaultService': return '已啟用 - 預設翻譯服務';
			case 'translation.notEnabled': return '未啟用';
			case 'translation.deeplxTranslationService': return 'DeepLX 翻譯服務';
			case 'translation.deeplxDescription': return 'DeepLX 是 DeepL 翻譯的開源實現，支援 Free、Pro 和 Official 三種端點模式';
			case 'translation.serverAddress': return '伺服器地址';
			case 'translation.serverAddressHint': return 'https://api.deeplx.org';
			case 'translation.serverAddressHelperText': return 'DeepLX 伺服器的基礎地址';
			case 'translation.endpointType': return '端點類型';
			case 'translation.freeEndpoint': return 'Free - 免費端點，可能有頻率限制';
			case 'translation.proEndpoint': return 'Pro - 需要 dl_session，更穩定';
			case 'translation.officialEndpoint': return 'Official - 官方 API 格式';
			case 'translation.finalRequestUrl': return '最終請求URL';
			case 'translation.apiKeyOptional': return 'API Key (可選)';
			case 'translation.apiKeyOptionalHint': return '用於訪問受保護的 DeepLX 服務';
			case 'translation.apiKeyOptionalHelperText': return '某些 DeepLX 服務需要 API Key 進行身份驗證';
			case 'translation.dlSession': return 'DL Session';
			case 'translation.dlSessionHint': return 'Pro 模式需要的 dl_session 參數';
			case 'translation.dlSessionHelperText': return 'Pro 端點必需的工作階段參數，從 DeepL Pro 帳戶獲取';
			case 'translation.proModeRequiresDlSession': return 'Pro 模式需要填寫 dl_session';
			case 'translation.clickTestButtonToVerifyDeepLXAPI': return '點擊測試按鈕驗證 DeepLX API 連接';
			case 'translation.enableDeepLXTranslation': return '啟用 DeepLX 翻譯';
			case 'translation.deepLXTranslationWillBeDisabled': return 'DeepLX翻譯將因配置更改而被停用';
			case 'translation.translatedResult': return '翻譯結果';
			case 'translation.testSuccess': return '測試成功';
			case 'translation.pleaseFillInDeepLXServerAddress': return '請填寫DeepLX伺服器地址';
			case 'translation.invalidAPIResponseFormat': return '無效的API響應格式';
			case 'translation.translationServiceReturnedError': return '翻譯服務返回錯誤或空結果';
			case 'translation.connectionFailed': return '連線失敗';
			case 'translation.translationFailed': return '翻譯失敗';
			case 'translation.aiTranslationFailed': return 'AI翻譯失敗';
			case 'translation.deeplxTranslationFailed': return 'DeepLX翻譯失敗';
			case 'translation.aiTranslationTestFailed': return 'AI翻譯測試失敗';
			case 'translation.deeplxTranslationTestFailed': return 'DeepLX翻譯測試失敗';
			case 'translation.streamingTranslationTimeout': return '流式翻譯超時，強制關閉資源';
			case 'translation.translationRequestTimeout': return '翻譯請求超時';
			case 'translation.streamingTranslationDataTimeout': return '流式翻譯接收資料超時';
			case 'translation.dataReceptionTimeout': return '接收資料超時';
			case 'translation.streamDataParseError': return '解析流資料時出錯';
			case 'translation.streamingTranslationFailed': return '流式翻譯失敗';
			case 'translation.fallbackTranslationFailed': return '降級到普通翻譯也失敗';
			case 'translation.translationSettings': return '翻譯設定';
			case 'translation.enableGoogleTranslation': return '啟用 Google 翻譯';
			...
			case 'linkInputDialog.supportedLinksHint': return ({required Object webName}) => '支援智慧識別多個${webName}連結，並快速跳轉到應用內對應頁面(連結與其他文字之間用空格隔開)';
			...
			case 'linkInputDialog.unsupportedLinkDialogContent': return '該連結類型目前應用無法直接打開，需要使用外部瀏覽器訪問。\n\n是否使用瀏覽器打開此連結？';
			...
			case 'log.logManagement': return '日誌管理';
			case 'log.enableLogPersistence': return '持久化日誌';
			case 'log.enableLogPersistenceDesc': return '將日誌儲存到資料庫以便於分析問題';
			case 'log.logDatabaseSizeLimit': return '日誌資料庫大小上限';
			case 'log.logDatabaseSizeLimitDesc': return ({required Object size}) => '目前: ${size}';
			case 'log.exportCurrentLogs': return '匯出目前日誌';
			case 'log.exportCurrentLogsDesc': return '匯出當天應用日誌以幫助開發者診斷問題';
			case 'log.exportHistoryLogs': return '匯出歷史日誌';
			case 'log.exportHistoryLogsDesc': return '匯出指定日期範圍內的日誌';
			case 'log.exportMergedLogs': return '匯出合併日誌';
			case 'log.exportMergedLogsDesc': return '匯出指定日期範圍內的合併日誌';
			case 'log.showLogStats': return '顯示日誌統計資訊';
			case 'log.logExportSuccess': return '日誌匯出成功';
			case 'log.logExportFailed': return ({required Object error}) => '日誌匯出失敗: ${error}';
			case 'log.showLogStatsDesc': return '查看各種類型日誌的統計資料';
			case 'log.logExtractFailed': return ({required Object error}) => '獲取日誌統計失敗: ${error}';
			case 'log.clearAllLogs': return '清理所有日誌';
			case 'log.clearAllLogsDesc': return '清理所有日誌資料';
			case 'log.confirmClearAllLogs': return '確認清理';
			case 'log.confirmClearAllLogsDesc': return '確定要清理所有日誌資料嗎？此操作不可復原。';
			case 'log.clearAllLogsSuccess': return '日誌清理成功';
			case 'log.clearAllLogsFailed': return ({required Object error}) => '清理日誌失敗: ${error}';
			case 'log.unableToGetLogSizeInfo': return '無法獲取日誌大小資訊';
			case 'log.currentLogSize': return '目前日誌大小:';
			case 'log.logCount': return '日誌數量:';
			...
			case 'log.currentLogSizeExceededPleaseCleanOldLogsOrIncreaseLogSizeLimit': return '日誌空間已超出限制，建議立即清理舊日誌或增加空間限制';
			case 'log.currentLogSizeAlmostExceededPleaseCleanOldLogs': return '日誌空間即將用盡，建議清理舊日誌';
			case 'log.cleaningOldLogs': return '正在自動清理舊日誌...';
			case 'log.logCleaningCompleted': return '日誌清理完成';
			case 'log.logCleaningProcessMayNotBeCompleted': return '日誌清理過程可能未完成';
			case 'log.cleanExceededLogs': return '清理超出限制的日誌';
			case 'log.noLogsToExport': return '沒有可匯出的日誌資料';
			case 'log.exportingLogs': return '正在匯出日誌...';
			case 'log.noHistoryLogsToExport': return '尚無可匯出的歷史日誌，請先使用應用一段時間再嘗試';
			case 'log.selectLogDate': return '選擇日誌日期';
			case 'log.today': return '今天';
			case 'log.selectMergeRange': return '選擇合併範圍';
			case 'log.selectMergeRangeHint': return '請選擇要合併的日誌時間範圍';
			case 'log.selectMergeRangeDays': return ({required Object days}) => '最近 ${days} 天';
			case 'log.logStats': return '日誌統計資訊';
			case 'log.todayLogs': return ({required Object count}) => '今日日誌: ${count} 條';
			case 'log.recent7DaysLogs': return ({required Object count}) => '最近7天: ${count} 條';
			case 'log.totalLogs': return ({required Object count}) => '總計日誌: ${count} 條';
			case 'log.setLogDatabaseSizeLimit': return '設定日誌資料庫大小上限';
			case 'log.currentLogSizeWithSize': return ({required Object size}) => '目前日誌大小: ${size}';
			...
			case 'log.logSizeLimitSetSuccess': return ({required Object size}) => '日誌大小上限已設定為 ${size}';
			...
			case 'emoji.noEmojis': return '暫無表情包';
			case 'emoji.clickToAddEmojis': return '點擊右上角按鈕添加表情包';
			case 'emoji.addEmojis': return '添加表情包';
			case 'emoji.imagePreview': return '圖片預覽';
			case 'emoji.imageLoadFailed': return '圖片載入失敗';
			case 'emoji.loading': return '載入中...';
			case 'emoji.delete': return '刪除';
			case 'emoji.close': return '關閉';
			case 'emoji.deleteImage': return '刪除圖片';
			case 'emoji.confirmDeleteImage': return '確定要刪除這張圖片嗎？';
			case 'emoji.cancel': return '取消';
			case 'emoji.batchDelete': return '批次刪除';
			case 'emoji.confirmBatchDelete': return ({required Object count}) => '確定要刪除選中的${count}張圖片嗎？此操作不可復原。';
			case 'emoji.deleteSuccess': return '成功刪除';
			case 'emoji.addImage': return '添加圖片';
			case 'emoji.addImageByUrl': return '透過URL添加';
			case 'emoji.addImageUrl': return '添加圖片URL';
			case 'emoji.imageUrl': return '圖片URL';
			case 'emoji.enterImageUrl': return '請輸入圖片URL';
			case 'emoji.add': return '添加';
			case 'emoji.batchImport': return '批次匯入';
			case 'emoji.enterJsonUrlArray': return '請輸入JSON格式的URL陣列:';
			case 'emoji.formatExample': return '格式範例:\n["url1", "url2", "url3"]';
			case 'emoji.pasteJsonUrlArray': return '請貼上JSON格式的URL陣列';
			case 'emoji.import': return '匯入';
			case 'emoji.importSuccess': return ({required Object count}) => '成功匯入${count}張圖片';
			case 'emoji.jsonFormatError': return 'JSON格式錯誤，請檢查輸入';
			case 'emoji.createGroup': return '建立表情包分組';
			case 'emoji.groupName': return '分組名稱';
			case 'emoji.enterGroupName': return '請輸入分組名稱';
			case 'emoji.create': return '建立';
			case 'emoji.editGroupName': return '編輯分組名稱';
			case 'emoji.save': return '儲存';
			case 'emoji.deleteGroup': return '刪除分組';
			case 'emoji.confirmDeleteGroup': return '確定要刪除這個表情包分組嗎？分組內的所有圖片也會被刪除。';
			case 'emoji.imageCount': return ({required Object count}) => '${count}張圖片';
			case 'emoji.selectEmoji': return '選擇表情包';
			case 'emoji.noEmojisInGroup': return '該分組暫無表情包';
			case 'emoji.goToSettingsToAddEmojis': return '前往設定添加表情包';
			case 'emoji.emojiManagement': return '表情包管理';
			case 'emoji.manageEmojiGroupsAndImages': return '管理表情包分組和圖片';
			case 'emoji.uploadLocalImages': return '上傳本機圖片';
			...
			case 'navigationOrderSettings.customNavigationOrderDesc': return '拖曳調整底部導航欄和側邊欄中各個頁面的顯示順序';
			...
			case 'navigationOrderSettings.subscriptionDescription': return '查看追蹤使用者的最新內容';
			...
			case 'anime4k.realTimeVideoUpscalingAndDenoising': return 'Anime4K 即時影片上採樣和降噪，提升動畫影片品質';
			case 'anime4k.settings': return 'Anime4K 設定';
			case 'anime4k.preset': return 'Anime4K 預設';
			case 'anime4k.disable': return '關閉 Anime4K';
			case 'anime4k.disableDescription': return '停用影片增強效果';
			case 'anime4k.highQualityPresets': return '高品質預設';
			case 'anime4k.fastPresets': return '快速預設';
			case 'anime4k.litePresets': return '輕量級預設';
			case 'anime4k.moreLitePresets': return '更多輕量級預設';
			case 'anime4k.customPresets': return '自訂預設';
			case 'anime4k.presetGroups.highQuality': return '高品質';
			case 'anime4k.presetGroups.fast': return '快速';
			case 'anime4k.presetGroups.lite': return '輕量級';
			case 'anime4k.presetGroups.moreLite': return '更多輕量級';
			case 'anime4k.presetGroups.custom': return '自訂';
			case 'anime4k.presetDescriptions.mode_a_hq': return '適用於大多數1080p動漫，特別是處理模糊、重採樣和壓縮瑕疵。提供最高的感知品質。';
			case 'anime4k.presetDescriptions.mode_b_hq': return '適用於輕微模糊或因縮放產生的振鈴效應的動漫。可以有效減少振鈴和鋸齒。';
			case 'anime4k.presetDescriptions.mode_c_hq': return '適用於幾乎沒有瑕疵的高品質片源（如原生1080p的動畫電影或桌布）。降噪並提供最高的PSNR。';
			case 'anime4k.presetDescriptions.mode_a_a_hq': return 'Mode A的強化版，提供極致的感知品質，能重建幾乎所有退化的線條。可能產生過度銳化或振鈴。';
			case 'anime4k.presetDescriptions.mode_b_b_hq': return 'Mode B的強化版，提供更高的感知品質，進一步最佳化線條和減少瑕疵。';
			case 'anime4k.presetDescriptions.mode_c_a_hq': return 'Mode C的感知品質增強版，在保持高PSNR的同時嘗試重建一些線條細節。';
			case 'anime4k.presetDescriptions.mode_a_fast': return 'Mode A的快速版本，平衡了品質與性能，適用於大多數1080p動漫。';
			case 'anime4k.presetDescriptions.mode_b_fast': return 'Mode B的快速版本，用於處理輕微瑕疵和振鈴，性能開銷較低。';
			case 'anime4k.presetDescriptions.mode_c_fast': return 'Mode C的快速版本，適用於高品質片源的快速降噪和放大。';
			case 'anime4k.presetDescriptions.mode_a_a_fast': return 'Mode A+A的快速版本，在性能有限的裝置上追求更高的感知品質。';
			case 'anime4k.presetDescriptions.mode_b_b_fast': return 'Mode B+B的快速版本，為性能有限的裝置提供增強的線條修復和瑕疵處理。';
			case 'anime4k.presetDescriptions.mode_c_a_fast': return 'Mode C+A的快速版本，在快速處理高品質片源的同時，進行輕度的線條修復。';
			case 'anime4k.presetDescriptions.upscale_only_s': return '僅使用最快的CNN模型進行x2放大，無修復和降噪，性能開銷最低。';
			case 'anime4k.presetDescriptions.upscale_deblur_fast': return '使用快速的非CNN演算法進行放大和去模糊，效果優於傳統演算法，性能開銷很低。';
			case 'anime4k.presetDescriptions.restore_s_only': return '僅使用最快的CNN模型修復畫面瑕疵，不進行放大。適用於原生解析度播放，但希望改善畫質的情況。';
			case 'anime4k.presetDescriptions.denoise_bilateral_fast': return '使用傳統的雙邊濾波器進行降噪，速度極快，適用於處理輕微噪點。';
			case 'anime4k.presetDescriptions.upscale_non_cnn': return '使用快速的傳統演算法進行放大，性能開銷極低，效果優於播放器內建演算法。';
			case 'anime4k.presetDescriptions.mode_a_fast_darken': return 'Mode A (Fast) + 線條加深，在快速模式A的基礎上增加線條加深效果，使線條更突出，風格化處理。';
			case 'anime4k.presetDescriptions.mode_a_hq_thin': return 'Mode A (HQ) + 線條細化，在高品質模式A的基礎上增加線條細化效果，讓畫面看起來更精緻。';
			...
			case 'anime4k.presetNames.restore_s_only': return '修復 (超快)';
			case 'anime4k.presetNames.denoise_bilateral_fast': return '雙邊降噪 (極快)';
			case 'anime4k.presetNames.upscale_non_cnn': return '非CNN放大 (極快)';
			case 'anime4k.presetNames.mode_a_fast_darken': return 'Mode A (Fast) + 線條加深';
			case 'anime4k.presetNames.mode_a_hq_thin': return 'Mode A (HQ) + 線條細化';
			case 'anime4k.performanceTip': return '💡 提示：根據裝置性能選擇合適的預設，低端裝置建議選擇輕量級預設。';
			default: return null;
		}
	}
}
