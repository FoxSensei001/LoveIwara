...
extension on TranslationsZhTw {
	dynamic _flatMapFunction(String path) {
		switch (path) {
			...
			case 'settings.fullscreenOrientation': return '豎屏進入全屏後的螢幕方向';
			case 'settings.fullscreenOrientationDesc': return '此設定決定豎屏進入全屏時螢幕的預設方向（僅移動端有效）';
			...
			case 'videoDetail.skeleton.successFecthVideoDurationInfo': return '成功獲取影片資源，開始加載影片...';
			case 'videoDetail.skeleton.successFecthVideoHeightInfo': return '加載完成';
			case 'videoDetail.cast.dlnaCast': return '投屏';
			case 'videoDetail.cast.unableToStartCastingSearch': return ({required Object error}) => '啟動投屏搜索失敗: ${error}';
			case 'videoDetail.cast.startCastingTo': return ({required Object deviceName}) => '開始投屏到 ${deviceName}';
			case 'videoDetail.cast.castFailed': return ({required Object error}) => '投屏失敗: ${error}\n請嘗試重新搜索設備或切換網絡';
			...
			case 'videoDetail.cast.deviceTypes.basicDevice': return '基礎設備';
			case 'videoDetail.cast.deviceTypes.dimmableLight': return '智能燈';
			case 'videoDetail.cast.deviceTypes.wlanAccessPoint': return '無線接入點';
			case 'videoDetail.cast.deviceTypes.wlanConnectionDevice': return '無線連接設備';
			case 'videoDetail.cast.deviceTypes.printer': return '打印機';
			case 'videoDetail.cast.deviceTypes.scanner': return '掃描儀';
			case 'videoDetail.cast.deviceTypes.digitalSecurityCamera': return '攝像頭';
			case 'videoDetail.cast.deviceTypes.unknownDevice': return '未知設備';
			case 'videoDetail.cast.currentPlatformNotSupported': return '當前平台不支援投屏功能';
			case 'videoDetail.cast.unableToGetVideoUrl': return '無法獲取影片地址，請稍後重試';
			case 'videoDetail.cast.stopCasting': return '停止投屏';
			case 'videoDetail.cast.dlnaCastSheet.title': return '遠程投屏';
			case 'videoDetail.cast.dlnaCastSheet.close': return '關閉';
			case 'videoDetail.cast.dlnaCastSheet.searchingDevices': return '正在搜索設備...';
			case 'videoDetail.cast.dlnaCastSheet.searchPrompt': return '點擊搜索按鈕重新搜索投屏設備';
			case 'videoDetail.cast.dlnaCastSheet.searching': return '搜索中';
			case 'videoDetail.cast.dlnaCastSheet.searchAgain': return '重新搜索';
			case 'videoDetail.cast.dlnaCastSheet.noDevicesFound': return '未發現投屏設備\n請確保設備在同一網絡下';
			case 'videoDetail.cast.dlnaCastSheet.searchingDevicesPrompt': return '正在搜索設備，請稍候...';
			...
			case 'videoDetail.cast.dlnaCastSheet.notConnected': return '未連接設備';
			...
			case 'translation.dlSessionHelperText': return 'Pro 端點必需的會話參數，從 DeepL Pro 帳戶獲取';
			...
			case 'translation.deepLXTranslationWillBeDisabled': return 'DeepLX翻譯將因配置更改而被禁用';
			...
			case 'translation.connectionFailed': return '連接失敗';
			...
			case 'translation.streamingTranslationDataTimeout': return '流式翻譯接收數據超時';
			case 'translation.dataReceptionTimeout': return '接收數據超時';
			case 'translation.streamDataParseError': return '解析流數據時出錯';
			...
			case 'emoji.batchDelete': return '批量刪除';
			case 'emoji.confirmBatchDelete': return ({required Object count}) => '確定要刪除選中的${count}張圖片嗎？此操作不可撤銷。';
			case 'emoji.deleteSuccess': return '成功刪除';
			case 'emoji.addImage': return '添加圖片';
			case 'emoji.addImageByUrl': return '通過URL添加';
			...
			case 'emoji.batchImport': return '批量導入';
			case 'emoji.enterJsonUrlArray': return '請輸入JSON格式的URL數組:';
			case 'emoji.formatExample': return '格式示例:\n["url1", "url2", "url3"]';
			case 'emoji.pasteJsonUrlArray': return '請粘貼JSON格式的URL數組';
			case 'emoji.import': return '導入';
			case 'emoji.importSuccess': return ({required Object count}) => '成功導入${count}張圖片';
			case 'emoji.jsonFormatError': return 'JSON格式錯誤，請檢查輸入';
			case 'emoji.createGroup': return '創建表情包分組';
			case 'emoji.groupName': return '分組名稱';
			case 'emoji.enterGroupName': return '請輸入分組名稱';
			case 'emoji.create': return '創建';
			...
			case 'anime4k.realTimeVideoUpscalingAndDenoising': return 'Anime4K 即時視頻上採樣和降噪，提升動畫視頻品質';
			case 'anime4k.settings': return 'Anime4K 設定';
			case 'anime4k.preset': return 'Anime4K 預設';
			case 'anime4k.disable': return '關閉 Anime4K';
			case 'anime4k.disableDescription': return '禁用視頻增強效果';
			...
			case 'anime4k.customPresets': return '自定義預設';
			...
			case 'anime4k.presetGroups.custom': return '自定義';
			...
			case 'anime4k.presetDescriptions.mode_c_hq': return '適用於幾乎沒有瑕疵的高品質片源（如原生1080p的動畫電影或壁紙）。降噪並提供最高的PSNR。';
			case 'anime4k.presetDescriptions.mode_a_a_hq': return 'Mode A的強化版，提供極致的感知品質，能重建幾乎所有退化的線條。可能產生過度銳化或振鈴。';
			case 'anime4k.presetDescriptions.mode_b_b_hq': return 'Mode B的強化版，提供更高的感知品質，進一步優化線條和減少瑕疵。';
			...
			case 'anime4k.presetDescriptions.mode_a_a_fast': return 'Mode A+A的快速版本，在性能有限的設備上追求更高的感知品質。';
			case 'anime4k.presetDescriptions.mode_b_b_fast': return 'Mode B+B的快速版本，為性能有限的設備提供增強的線條修復和瑕疵處理。';
			...
			case 'anime4k.presetDescriptions.upscale_deblur_fast': return '使用快速的非CNN算法進行放大和去模糊，效果優於傳統算法，性能開銷很低。';
			case 'anime4k.presetDescriptions.restore_s_only': return '僅使用最快的CNN模型修復畫面瑕疵，不進行放大。適用於原生分辨率播放，但希望改善畫質的情況。';
			case 'anime4k.presetDescriptions.denoise_bilateral_fast': return '使用傳統的雙邊濾波器進行降噪，速度極快，適用於處理輕微噪點。';
			case 'anime4k.presetDescriptions.upscale_non_cnn': return '使用快速的傳統算法進行放大，性能開銷極低，效果優於播放器自帶算法。';
			...
			case 'anime4k.performanceTip': return '💡 提示：根據設備性能選擇合適的預設，低端設備建議選擇輕量級預設。';
			default: return null;
		}
	}
}
