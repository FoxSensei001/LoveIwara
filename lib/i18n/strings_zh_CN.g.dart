...
class _TranslationsSettingsZhCn extends TranslationsSettingsEn {
	...
	@override String get glassEffectIntroContent => '现在顶栏、底栏和菜单用的是液态玻璃（真实的模糊与折射）。如果设备上觉得卡或更喜欢简洁，可以现在换成 Material（不透明、无模糊、无阴影）。';
	@override String get glassEffectIntroHint => '以后随时可以在「设置 → 主题设置 → 界面材质」里改。';
...
class _TranslationsExternalPlayerZhCn extends TranslationsExternalPlayerEn {
	...
	@override String get managePlayersDescWindows => 'HereSphere、DeoVR、Whirligig 这类 PCVR 播放器不是系统默认关联程序，指定它们的 .exe 之后，就能从播放器里把当前视频直接转交过去';
	@override String get managePlayersDescMac => '指定 IINA、VLC、mpv 这类播放器的应用程序，之后就能从播放器里把当前视频直接转交过去';
	@override String get managePlayersDescLinux => '指定 mpv、VLC、Celluloid 这类播放器的可执行文件，之后就能从播放器里把当前视频直接转交过去';
	@override String get pickExecutableHintWindows => '选播放器安装目录里的 .exe 主程序，例如 HereSphere.exe、vlc.exe。桌面快捷方式（.lnk）不行';
	@override String get pickExecutableHintMac => '在「应用程序」里选播放器的 .app，例如 IINA.app；里面真正的可执行文件会自动定位';
	@override String get pickExecutableHintLinux => '选播放器的可执行文件，例如 /usr/bin/mpv；用 which mpv 就能查到它在哪儿';
	@override String emptyStateGuide({required Object examples}) => '配好之后，播放页的「用其他应用打开」里会直接多出对应的条目。常见的有：${examples}';
	@override String get detectNothingFoundGuide => '没探测到已安装的播放器。装在自定义目录、或者是免安装的绿色版都探不到，用「添加播放器」手动指定即可';
	@override String get detectNothingNew => '没有发现新的播放器，装了的都已经在列表里了';
	@override String get detectFailed => '探测失败，可以改用「添加播放器」手动指定';
	@override String get advancedOptions => '高级选项';
	@override String get playerNameHint => '留空就用文件名';
	@override String get executablePathRequired => '请先选择播放器的可执行文件';
...
class _TranslationsWatchLaterZhCn extends TranslationsWatchLaterEn {
	...
	@override String removedCount({required Object count}) => '已移出 ${count} 条';
...
class _TranslationsPlaybackQueueZhCn extends TranslationsPlaybackQueueEn {
	...
	@override String get emptyGalleryQueue => '这个池里没有可看的图库';
	...
	@override String get playNext => '下一个';
	@override String get queueEnded => '已经是这个池里的最后一条了';
	@override String get playNextHint => '点击播放下一条，长按打开「接着看」';
	@override String get authorVideos => '作者的视频';
	@override String get authorGalleries => '作者的图库';
...
class _TranslationsLocalMediaZhCn extends TranslationsLocalMediaEn {
	...
	@override String addedAsPinnedFolder({required Object name, required Object source}) => '「${name}」在「${source}」里面，已加到常用目录';
	@override String alreadyPinnedFolder({required Object name}) => '「${name}」已经在常用目录里了';
	@override String sourceAlreadyAdded({required Object name}) => '已经添加过「${name}」';
...
// Path: historyPage
class _TranslationsHistoryPageZhCn extends TranslationsHistoryPageEn {
	_TranslationsHistoryPageZhCn._(TranslationsZhCn root) : this._root = root, super.internal(root);

	final TranslationsZhCn _root; // ignore: unused_field

	// Translations
	@override String get removeFromHistory => '从历史中删除';
	@override String get removed => '已从历史中删除';
...
class _TranslationsAiZhCn extends TranslationsAiEn {
	...
	@override String searchToolFound({required Object count, required Object titles}) => '约 ${count} 条 · ${titles}';
	@override String searchToolFailed({required Object reason}) => '试搜失败：${reason}';
	@override String searchToolFilterCount({required Object count}) => '${count} 个筛选';
	@override String searchToolExpanded({required Object tags}) => '按 ${tags} 补搜';
	@override String searchToolLookupTags({required Object terms}) => '查标签 ${terms}';
...
extension on TranslationsZhCn {
	dynamic _flatMapFunction(String path) {
		return switch (path) {
			...
			'settings.glassEffectIntroContent' => '现在顶栏、底栏和菜单用的是液态玻璃（真实的模糊与折射）。如果设备上觉得卡或更喜欢简洁，可以现在换成 Material（不透明、无模糊、无阴影）。',
			'settings.glassEffectIntroHint' => '以后随时可以在「设置 → 主题设置 → 界面材质」里改。',
			...
			'externalPlayer.managePlayersDescWindows' => 'HereSphere、DeoVR、Whirligig 这类 PCVR 播放器不是系统默认关联程序，指定它们的 .exe 之后，就能从播放器里把当前视频直接转交过去',
			'externalPlayer.managePlayersDescMac' => '指定 IINA、VLC、mpv 这类播放器的应用程序，之后就能从播放器里把当前视频直接转交过去',
			'externalPlayer.managePlayersDescLinux' => '指定 mpv、VLC、Celluloid 这类播放器的可执行文件，之后就能从播放器里把当前视频直接转交过去',
			'externalPlayer.pickExecutableHintWindows' => '选播放器安装目录里的 .exe 主程序，例如 HereSphere.exe、vlc.exe。桌面快捷方式（.lnk）不行',
			'externalPlayer.pickExecutableHintMac' => '在「应用程序」里选播放器的 .app，例如 IINA.app；里面真正的可执行文件会自动定位',
			'externalPlayer.pickExecutableHintLinux' => '选播放器的可执行文件，例如 /usr/bin/mpv；用 which mpv 就能查到它在哪儿',
			'externalPlayer.emptyStateGuide' => ({required Object examples}) => '配好之后，播放页的「用其他应用打开」里会直接多出对应的条目。常见的有：${examples}',
			'externalPlayer.detectNothingFoundGuide' => '没探测到已安装的播放器。装在自定义目录、或者是免安装的绿色版都探不到，用「添加播放器」手动指定即可',
			'externalPlayer.detectNothingNew' => '没有发现新的播放器，装了的都已经在列表里了',
			'externalPlayer.detectFailed' => '探测失败，可以改用「添加播放器」手动指定',
			'externalPlayer.advancedOptions' => '高级选项',
			'externalPlayer.playerNameHint' => '留空就用文件名',
			'externalPlayer.executablePathRequired' => '请先选择播放器的可执行文件',
			...
			'playbackQueue.emptyGalleryQueue' => '这个池里没有可看的图库',
			...
			'playbackQueue.playNext' => '下一个',
			'playbackQueue.queueEnded' => '已经是这个池里的最后一条了',
			'playbackQueue.playNextHint' => '点击播放下一条，长按打开「接着看」',
			'playbackQueue.authorVideos' => '作者的视频',
			'playbackQueue.authorGalleries' => '作者的图库',
			...
			'localMedia.addedAsPinnedFolder' => ({required Object name, required Object source}) => '「${name}」在「${source}」里面，已加到常用目录',
			'localMedia.alreadyPinnedFolder' => ({required Object name}) => '「${name}」已经在常用目录里了',
			'localMedia.sourceAlreadyAdded' => ({required Object name}) => '已经添加过「${name}」',
			...
			'ai.searchToolFound' => ({required Object count, required Object titles}) => '约 ${count} 条 · ${titles}',
			'ai.searchToolFailed' => ({required Object reason}) => '试搜失败：${reason}',
			'ai.searchToolFilterCount' => ({required Object count}) => '${count} 个筛选',
			'ai.searchToolExpanded' => ({required Object tags}) => '按 ${tags} 补搜',
			'ai.searchToolLookupTags' => ({required Object terms}) => '查标签 ${terms}',
