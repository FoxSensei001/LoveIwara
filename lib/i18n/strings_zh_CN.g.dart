...
extension on TranslationsZhCn {
	dynamic _flatMapFunction(String path) {
		switch (path) {
			...
			case 'common.createTimeDesc': return '创建时间倒序';
			case 'common.createTimeAsc': return '创建时间正序';
			...
			case 'settings.enableHorizontalDragSeek': return '横向滑动调整进度';
			...
			case 'settings.cdnDistributionStrategy': return '内容分发策略';
			case 'settings.cdnDistributionStrategyDesc': return '选择视频源服务器的分发策略，可优化加载速度';
			case 'settings.cdnDistributionStrategyLabel': return '分发策略';
			case 'settings.cdnDistributionStrategyNoChange': return '不修改（使用原服务器）';
			case 'settings.cdnDistributionStrategyAuto': return '自动选择（最快服务器）';
			case 'settings.cdnDistributionStrategySpecial': return '指定服务器';
			case 'settings.cdnSpecialServer': return '指定服务器';
			case 'settings.cdnRefreshServerListHint': return '请先点击下方按钮刷新服务器列表';
			case 'settings.cdnRefreshButton': return '刷新';
			case 'settings.cdnFastRingServers': return '快速环服务器';
			case 'settings.cdnRefreshServerListTooltip': return '刷新服务器列表';
			case 'settings.cdnSpeedTestButton': return '测速';
			case 'settings.cdnSpeedTestingButton': return ({required Object count}) => '测速中 (${count})';
			case 'settings.cdnNoServerDataHint': return '暂无服务器数据，请点击刷新按钮';
			case 'settings.cdnTestingStatus': return '测速中';
			case 'settings.cdnUnreachableStatus': return '不可达';
			case 'settings.cdnNotTestedStatus': return '未测速';
			...
			case 'subscriptions.showAllSubscribedUsersContent': return '显示所有已订阅用户的内容';
			...
			case 'videoDetail.likeAvatars.dialogTitle': return '谁在偷偷喜欢';
			case 'videoDetail.likeAvatars.dialogDescription': return '好奇他们是谁？翻翻这本「点赞相册」吧～';
			case 'videoDetail.likeAvatars.closeTooltip': return '关闭';
			case 'videoDetail.likeAvatars.retry': return '重试';
			case 'videoDetail.likeAvatars.noLikesYet': return '还没有人出现在这里，来当第一个吧！';
			case 'videoDetail.likeAvatars.pageInfo': return ({required Object page, required Object totalPages, required Object totalCount}) => '第 ${page} / ${totalPages} 页 · 共 ${totalCount} 人';
			case 'videoDetail.likeAvatars.prevPage': return '上一页';
			case 'videoDetail.likeAvatars.nextPage': return '下一页';
			...
			case 'download.errors.sslHandshakeFailed': return 'SSL握手失败，请检查网络环境';
			case 'download.errors.connectionFailed': return '连接失败，请检查网络';
			...
			case 'download.viewDownloadList': return '查看下载列表';
			...
			case 'download.alreadyDownloadedWithQuality': return '已有相同清晰度的任务，是否继续下载？';
			case 'download.alreadyDownloadedWithQualities': return ({required Object qualities}) => '已有清晰度为${qualities}的任务，是否继续下载？';
			...
			case 'translation.currentService': return '当前服务';
			...
			case 'translation.translationService': return '翻译服务';
			case 'translation.translationServiceDescription': return '选择您偏好的翻译服务';
			case 'translation.googleTranslation': return 'Google 翻译';
			case 'translation.googleTranslationDescription': return '免费的在线翻译服务，支持多种语言';
			case 'translation.aiTranslation': return 'AI 翻译';
			case 'translation.aiTranslationDescription': return '基于大语言模型的智能翻译服务';
			case 'translation.deeplxTranslation': return 'DeepLX 翻译';
			case 'translation.deeplxTranslationDescription': return 'DeepL 翻译的开源实现，提供高质量翻译';
			case 'translation.googleTranslationFeatures': return '特性';
			case 'translation.freeToUse': return '免费使用';
			case 'translation.freeToUseDescription': return '无需配置，开箱即用';
			case 'translation.fastResponse': return '快速响应';
			case 'translation.fastResponseDescription': return '翻译速度快，延迟低';
			case 'translation.stableAndReliable': return '稳定可靠';
			case 'translation.stableAndReliableDescription': return '基于Google官方API';
			case 'translation.enabledDefaultService': return '已启用 - 默认翻译服务';
			case 'translation.notEnabled': return '未启用';
			case 'translation.deeplxTranslationService': return 'DeepLX 翻译服务';
			case 'translation.deeplxDescription': return 'DeepLX 是 DeepL 翻译的开源实现，支持 Free、Pro 和 Official 三种端点模式';
			case 'translation.serverAddress': return '服务器地址';
			case 'translation.serverAddressHint': return 'https://api.deeplx.org';
			case 'translation.serverAddressHelperText': return 'DeepLX 服务器的基础地址';
			case 'translation.endpointType': return '端点类型';
			case 'translation.freeEndpoint': return 'Free - 免费端点，可能有频率限制';
			case 'translation.proEndpoint': return 'Pro - 需要 dl_session，更稳定';
			case 'translation.officialEndpoint': return 'Official - 官方 API 格式';
			case 'translation.finalRequestUrl': return '最终请求URL';
			case 'translation.apiKeyOptional': return 'API Key (可选)';
			case 'translation.apiKeyOptionalHint': return '用于访问受保护的 DeepLX 服务';
			case 'translation.apiKeyOptionalHelperText': return '某些 DeepLX 服务需要 API Key 进行身份验证';
			case 'translation.dlSession': return 'DL Session';
			case 'translation.dlSessionHint': return 'Pro 模式需要的 dl_session 参数';
			case 'translation.dlSessionHelperText': return 'Pro 端点必需的会话参数，从 DeepL Pro 账户获取';
			case 'translation.proModeRequiresDlSession': return 'Pro 模式需要填写 dl_session';
			case 'translation.clickTestButtonToVerifyDeepLXAPI': return '点击测试按钮验证 DeepLX API 连接';
			case 'translation.enableDeepLXTranslation': return '启用 DeepLX 翻译';
			case 'translation.deepLXTranslationWillBeDisabled': return 'DeepLX翻译将因配置更改而被禁用';
			case 'translation.translatedResult': return '翻译结果';
			case 'translation.testSuccess': return '测试成功';
			case 'translation.pleaseFillInDeepLXServerAddress': return '请填写DeepLX服务器地址';
			case 'translation.invalidAPIResponseFormat': return '无效的API响应格式';
			case 'translation.translationServiceReturnedError': return '翻译服务返回错误或空结果';
			case 'translation.connectionFailed': return '连接失败';
			case 'translation.translationFailed': return '翻译失败';
			case 'translation.aiTranslationFailed': return 'AI翻译失败';
			case 'translation.deeplxTranslationFailed': return 'DeepLX翻译失败';
			case 'translation.aiTranslationTestFailed': return 'AI翻译测试失败';
			case 'translation.deeplxTranslationTestFailed': return 'DeepLX翻译测试失败';
			case 'translation.streamingTranslationTimeout': return '流式翻译超时，强制关闭资源';
			case 'translation.translationRequestTimeout': return '翻译请求超时';
			case 'translation.streamingTranslationDataTimeout': return '流式翻译接收数据超时';
			case 'translation.dataReceptionTimeout': return '接收数据超时';
			case 'translation.streamDataParseError': return '解析流数据时出错';
			case 'translation.streamingTranslationFailed': return '流式翻译失败';
			case 'translation.fallbackTranslationFailed': return '降级到普通翻译也失败';
			case 'translation.translationSettings': return '翻译设置';
			case 'translation.enableGoogleTranslation': return '启用 Google 翻译';
			...
			case 'emoji.uploadLocalImages': return '上传本地图片';
			case 'emoji.uploadingImages': return '正在上传图片';
			case 'emoji.uploadingImagesProgress': return ({required Object count}) => '正在上传 ${count} 张图片，请稍候...';
			case 'emoji.doNotCloseDialog': return '请不要关闭此对话框';
			case 'emoji.uploadSuccess': return ({required Object count}) => '成功上传 ${count} 张图片';
			...
			case 'searchFilter.sortTypes.relevance': return '相关性';
