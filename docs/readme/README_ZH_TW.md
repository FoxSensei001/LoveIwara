...
[English](../../README.md) · [日本語](README_JA.md) · [简体中文](README_ZH.md) · **繁體中文** · [한국어](README_KO.md) · [ภาษาไทย](README_TH.md) · [Bahasa Indonesia](README_ID.md) · [Tiếng Việt](README_VI.md) · [Español](README_ES.md) · [Русский](README_RU.md) · [Français](README_FR.md) · [Deutsch](README_DE.md)
...
**整個應用程式住在一個空間環境裡。** 從 Quest 主畫面點開就直接進入常駐的沉浸空間。應用程式的全部——瀏覽、搜尋、留言、軟鍵盤——作為一塊 2D 面板懸浮在你面前，沒有任何功能被裁減。這塊面板是**微曲面**的（30° 弧度，預設 1.6 m 寬時約合「3000R」那一檔曲面螢幕）：定死的是弧度而不是半徑，所以視窗拉寬拉窄，彎的程度看上去始終一致。
...
- *控制器*——A/X 播放暫停，B/Y 收起面板或返回應用程式，Menu 開啟設定。握把扳機不用瞄準就能抓住幕布。搖桿左右拖曳進度並帶加速（輕推一下是 5 秒，按住能一路加到每秒 15 分鐘），上下把幕布推遠或拉近。
- 摘下頭戴裝置自動暫停、戴回來繼續；系統重新定位會把所有東西重新擺到你面前。

**第一次進來會先上一課。** 頭戴裝置上的首次導覽不是把觸控那套「雙指捏合、長按倍速」照搬過來——空間裡根本沒有可觸控的畫面。取而代之的是**空間影片**與**空間畫廊**兩組課程：控制器貼圖跟著按鍵與搖桿逐格動，幕布、面板與射線按真實幾何擺位，控制器與裸手各示範一遍。系統開了「減弱動態效果」就只畫一格有資訊量的靜止圖；設定裡隨時可以重看。
...
介面目前支援 **12 種語言**；除了最初那四種（English / 简体中文 / 繁體中文 / 日本語，已經人工校對）之外，其餘都是機器翻譯，還沒逐條人工校對。如果你願意幫忙改進，請從簡體中文範本開始：[`lib/i18n/zh-CN.i18n.yaml`](../../lib/i18n/zh-CN.i18n.yaml)，接著執行 `dart run slang`。完整的各語言狀態與維護指令見 **[`docs/i18n/README.md`](../i18n/README.md)**。
...
<a href="https://github.com/FoxSensei001/LoveIwara/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=FoxSensei001/LoveIwara&max=100" alt="專案貢獻者" />
</a>
